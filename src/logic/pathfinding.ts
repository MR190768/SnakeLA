...
export const findNearestFoodDistance = (startCoord: Coord, gameState: GameState): number => {
  ...
  for (const snake of board.snakes) {
    ...
    for (let i = 0; i < limit; i++) {
      obstacleSet.add(`${snake.body[i].x},${snake.body[i].y}`);
    }
  }
  ...
  while (queue.length > 0) {
    ...
    if (foodSet.has(`${coord.x},${coord.y}`)) {
      return dist;
    }
    
    if (dist > 30) return Infinity; // Prevent deep searches
    ...
    for (const dir of directions) {
      ...
      if (
        !visited.has(key) &&
        dir.x >= 0 && dir.x < board.width &&
        dir.y >= 0 && dir.y < board.height &&
        !obstacleSet.has(key)
      ) {
        visited.add(key);
